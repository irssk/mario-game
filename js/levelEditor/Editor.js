...
function Editor() {
  ...
  this.init = function() {
    ...
    that.drawGrid(3840); // при запуске по умолчанию рисует сетку размером 3840 пикселей
    that.showElements();
  };
    ...
    for (i = 1; i <= row; i++) {
      ...
      for (j = 1; j <= column; j++) {
        ...
        td.addEventListener('mousedown', function(e) {
          e.preventDefault(); // остановить указатель мыши, чтобы измениться
        });
      ...
      grid.onmouseleave = function() {
        // если мышь наведена на экран редактора
        mousedown = false;
      };
    }
  ...
  this.showElements = function() {
    ...
    // для каждого элемента в массиве elements он создает div и устанавливает имя класса
  ...
  that.gridSmall = function() {
    ...
    that.drawGrid(1280); // небольшой размер сетки
  };

  that.gridMedium = function() {
    view.remove(gameWorld, grid);
    that.drawGrid(3840); // средний размер сетки
  };

  that.gridLarge = function() {
    view.remove(gameWorld, grid);
    that.drawGrid(6400); // большой размер сетки
  };

  this.drawElement = function(element) {
    /*
      каждый выбранный элемент помещается в массив 'selectedElement'
      после щелчка по нужному элементу он проходит по массиву и устанавливает имя класса
      этой ячейки, изменяя фон ячейки.
    */
  ...
  that.generateMap = function() {
    ...
    // циклически перебирает ячейки таблицы и проверяет имя класса, помещает значение в соответствии с его именем класса;
  ...
  this.saveMap = function() {
    ...
    // для исправления сортировки localStorage, 01 02...10 11, иначе сортировка была бы 1 10 11.. 2 20 21..
    if (levelCounter < 10) {
    ...
    console.log(storage.getItem(levelName)); // для копирования сгенерированной карты при необходимости
  };
